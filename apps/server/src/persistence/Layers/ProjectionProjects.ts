...
const ProjectionProjectDbRow = ProjectionProject.mapFields(
  Struct.assign({
    ...
    copyFilePatterns: Schema.fromJsonString(Schema.Array(Schema.String)),
  }),
);
    ...
    execute: (row) =>
      sql`
        ...
        VALUES (
          ...
          ${JSON.stringify(row.copyFilePatterns)},
          ${row.createdAt},
          ${row.updatedAt},
          ${row.deletedAt}
        )
        ...
        DO UPDATE SET
          ...
          copy_file_patterns_json = excluded.copy_file_patterns_json,
    ...
    execute: ({ projectId }) =>
      sql`
        SELECT
          ...
          COALESCE(copy_file_patterns_json, '[]') AS "copyFilePatterns",
