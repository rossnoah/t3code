...
export interface ProjectScriptEditorDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Script being edited; null for the add flow. */
...
/**
 * Add/edit dialog for a project script (action), shared by the chat header
 * scripts control and the Repos settings section.
 */
export function ProjectScriptEditorDialog({
  ...
  onUpdateScript,
  onDeleteScript,
}: ProjectScriptEditorDialogProps) {
  ...
  useEffect(() => {
    ...
    setIcon(editingScript?.icon ?? "play");
    setIconPickerOpen(false);
    setRunOnWorktreeCreate(editingScript?.runOnWorktreeCreate ?? false);
    setKeybinding(
      editingScript
    ...
    // Seed the form only when the dialog opens; keybindings churn shouldn't reset edits.
  }, [open, editingScript]);
  ...
  const submitAddScript = async (event: FormEvent) => {
    ...
    const result = editingScript
      ? await onUpdateScript(editingScript.id, payload)
  ...
  const confirmDeleteScript = useCallback(() => {
    ...
    onOpenChange(false);
    void onDeleteScript(editingScript.id);
  }, [editingScript, onDeleteScript, onOpenChange]);
