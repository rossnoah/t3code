...
export const ComposerPrimaryActions = memo(function ComposerPrimaryActions({
  ...
  // Shared send affordance. It renders on its own when idle and next to the Stop
  // button while the agent runs, so you can send a follow-up or stop mid-turn.
  // (Tab, separately, stages a message into the queue.)
  const sendButton = (
    <button
      type="submit"
  ...
  if (isRunning) {
    // Both actions are available while the agent works: Stop interrupts the
    // current turn; Send stages the composed message into the queue.
    return (
      <div className={cn("flex items-center justify-end", compact ? "gap-1.5" : "gap-2")}>
        <button
          type="button"
